...
class AIRecommendationEngine:
    ...
    def prepare_course_features(self, courses):
        ...
        try:
            # Extract text features from courses
            course_texts = []
            for course in courses:
                text = f"{course.get('course_name', '')} {course.get('description', '')} {course.get('level', '')} {course.get('category', '')}"
    ...
    def get_course_recommendations(self, student_id, student_data, enrolled_courses, completed_courses, limit=5):
        ...
        try:
            ...
            for i, course in enumerate(self.course_features):
                if course['id'] in [c['id'] for c in enrolled_courses + completed_courses]:
                    continue  # Skip already taken courses
                
                score = self._calculate_course_score(course, learning_profile, i)
            ...
            self._store_recommendations(student_id, recommendations)
    ...
    def _analyze_learning_profile(self, student_id, enrolled_courses, completed_courses):
        ...
        try:
            ...
            if completed_courses:
                ...
                levels = [c.get('level', 'Beginner') for c in completed_courses]
                categories = [c.get('category', '') for c in completed_courses]
    ...
    def _calculate_course_score(self, course, learning_profile, course_index):
        ...
        try:
            ...
            similarity_score = 0
            if self.course_vectors is not None and course_index < self.course_vectors.shape[0]:
                # Calculate similarity with completed courses
                current_vector = self.course_vectors[course_index]
                # This would be enhanced with actual similarity calculations
    ...
    def _store_recommendations(self, student_id, recommendations):
        """Store recommendations in database for tracking"""
        try:
            # This would store recommendations for analytics
...
recommendation_engine = AIRecommendationEngine(None)  # Will be set when database is available
